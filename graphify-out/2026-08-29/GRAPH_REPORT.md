# Graph Report - SIH26  (2026-08-28)

## Corpus Check
- 85 files · ~98,617 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1277 nodes · 1611 edges · 110 communities (94 shown, 16 thin omitted)
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 223 edges (avg confidence: 0.85)
...
### Community 5 - "03 — Tech Stack & Model Selection"
Cohesion: 0.15
Nodes (13): 03 — Tech Stack & Model Selection, 1.1 Client-Side Models (run in browser), 1.2 Server-Side VLM (runs on cloud/self-hosted GPU), 1. Model Selection Matrix, 2. Architecture Decision Records, 3. Runtime Stack, 4. Latency Budget, 5. Resource Budget (+5 more)
...
Cohesion: 0.09
Nodes (21): 01 — Requirements, 1. Functional Requirements, 2. Non-Functional Requirements, 3. Redaction Taxonomy, 4. MVP Scope vs. Stretch Goals, 5. Traceability Matrix (Requirement → Scoring Criterion), FR-01 — Screen Capture, FR-02 — DOM-Based Field Detection (Layer 1) (+13 more)
...
Cohesion: 0.12
Nodes (28): ACTION_LIMITS, assertReadyForVlm(), buildPageStructureForVlm(), buildVlmAuthHeaders(), ensureOffscreen(), escapeRegex(), FORBIDDEN_LOCAL_SECRET_KEYS, handleCaptureAndSanitize() (+20 more)
...
Cohesion: 0.18
Nodes (11): 1. 🔎 Pain Points & Core Understanding, 2. ⚙️ Feasibility of Execution, 3. 🌍 Impact & Relevance, 5. 🧩 Clarity of Problem Statement, 6. 🎯 Evaluator's Perspective, 7. 👥 Strategy for Team Fit & Execution, 8. 🤖 AI-Buildability Split (20/80), 9. 📊 Data & Resource Availability (+3 more)
...
Cohesion: 0.22
Nodes (7): fs, PAGE_FIELDS, path, sandbox, source, SRC_PATH, vm
...
Cohesion: 0.18
Nodes (10): 2026-08-28 — Follow-up after demo pack 3.1–3.6, 2026-08-28 — Follow-up after post-reload content-script inject, 2026-08-28 — Follow-up after Task 2.8 (face-before-VLM), 2026-08-28 — Follow-up after Task 2.9 (NER-before-VLM), Codebase Maintenance Work Report, Critical finding — Phase-2 work described in engineer reports is not present in `src/`, Doc-vs-code contradictions found (reported, not edited), Files changed (+2 more)
...
Cohesion: 0.10
Nodes (18): BACKGROUND_PATH, backgroundSrc, CONTENT_PATH, contentSrc, contract, CONTRACT_PATH, fs, localSets (+10 more)
...
Cohesion: 0.20
Nodes (10): 02 — Architecture, 1. System Overview, 3.1 End-to-End Sequence, 3.2 Detection → Bounding Box Mapping, 3. Data Flow — Sanitization Pipeline, 4.1 Request Format, 4.2 Response Format (parsed from VLM output), 4.3 Key Design Principle (+2 more)
...
### Community 101 - "Aegis — On-device Visual Perception for Light-weight Browser Agents"
Cohesion: 0.33
Nodes (6): Aegis — On-device Visual Perception for Light-weight Browser Agents, Evaluation Criteria (5 Explicitly Weighted), Key Technical Decisions (Summary), Planning Documents, Project Overview, Research Documents (from analysis phase)

### Community 102 - "10. 🎤 Judge Q&A Stress-Test"
Cohesion: 0.33
Nodes (6): 10. 🎤 Judge Q&A Stress-Test, ❓ Q1: "Show me your model's actual latency and memory usage numbers — not an estimate, real measured data.", ❓ Q2: "Your redaction worked well on your demo page — what happens on a page you haven't tested, with a different layout?", ❓ Q3: "The problem statement says redaction should be 'dynamic' — what does that actually mean in your implementation, versus a static rule set?", ❓ Q4: "How does the server know how to interpret your sanitized/redacted context correctly, given it never sees the real data?", ❓ Q5: "This whole system still trusts the server not to log or misuse the sanitized data it does receive — how is that fundamentally different from just trusting the server with everything?"

### Community 103 - "2. Component Breakdown"
Cohesion: 0.40
Nodes (5): 2.1 Content Script (`content.js`), 2.2 Service Worker (`background.js`), 2.3 Offscreen Document (`offscreen.html`), 2.4 Inference Worker (inside offscreen document), 2. Component Breakdown

### Community 104 - "5. WASM-First / WebGPU-Accelerated Strategy"
Cohesion: 0.50
Nodes (4): 5.1 Runtime Backend Selection, 5.2 Why WASM-First, 5.3 Model Weight Caching, 5. WASM-First / WebGPU-Accelerated Strategy

### Community 105 - "4. 💡 Scope of Innovation (Existing Solutions)"
Cohesion: 0.50
Nodes (4): 4. 💡 Scope of Innovation (Existing Solutions), 🏆 Competitor / Existing Solution Landscape, 🕳️ The Whitespace, 🚀 What to Add / Stand Out Technically

## Knowledge Gaps
- **724 isolated node(s):** `fs`, `path`, `vm`, `ROOT`, `BACKGROUND_PATH` (+719 more)
...
- **Why does `ortWasmThreaded()` connect `ortWasmThreaded` to `ort-wasm-simd-threaded.mjs`?**
